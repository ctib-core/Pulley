...
contract PulleyTokenEngineFuzzTest is Test {
    ...
    function testFuzz_ProvideLiquidity(uint256 amount) public {
        ...
        assertEq(
            pulleyTokenEngine.getAssetReserve(address(mockUSDC)),
            initialAssetReserve + amount,
            "Asset reserves should increase by amount"
        );
        assertEq(
            pulleyTokenEngine.totalBackingValue(),
            initialTotalBacking + amount,
            "Total backing should increase by amount"
        );
    ...
    function testFuzz_WithdrawLiquidity(uint256 depositAmount, uint256 withdrawAmount) public {
        ...
        assertLe(
            pulleyTokenEngine.getAssetReserve(address(mockUSDC)),
            initialAssetReserve,
            "Asset reserves should not increase"
        );
    ...
    function testFuzz_CoverTradingLoss(uint256 depositAmount, uint256 lossAmount) public {
        ...
        if (coverageResult) {
            ...
            assertGe(
                pulleyTokenEngine.totalLossesCovered(),
                initialTotalLossesCovered,
                "Total losses covered should increase"
            );
            assertLe(
                pulleyToken.insuranceFunds(), initialInsuranceFunds, "Insurance funds should decrease or stay same"
            );
        }
    }
    ...
    function testFuzz_MultiUserLiquidity(uint256 amount1, uint256 amount2, uint256 amount3) public {
        ...
        assertEq(
            pulleyTokenEngine.getAssetReserve(address(mockUSDC)),
            totalExpectedReserves,
            "Total reserves should match sum of deposits"
        );
        assertEq(
            pulleyTokenEngine.totalBackingValue(), totalExpectedTokens, "Total backing should match sum of deposits"
        );
    ...
    function testFuzz_InsuranceBackingMinter(uint256 amount) public {
        ...
        // Check invariants
        assertEq(
            pulleyTokenEngine.totalInsurancebacking(),
            initialInsuranceBacking + amount,
            "Insurance backing should increase"
        );
        assertEq(
            pulleyTokenEngine.getAssetReserve(address(mockUSDC)),
            initialAssetReserve + amount,
            "Asset reserves should increase"
        );
