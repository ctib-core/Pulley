...
contract PulleyEcosystemHandler is Test {
    ...
    function buyTokensAndDeposit(uint256 actorSeed, uint256 tokenAmount, uint256 tradingAmount) public useActor(actorSeed) {
...
contract PulleyEcosystemInvariantTest is StdInvariant, Test {
    ...
    function setUp() public {
        ...
        handler = new PulleyEcosystemHandler(
            gateway,
            pulleyToken,
            pulleyTokenEngine,
            tradingPool,
    ...
    function invariant_PulleyTokenSupplyEqualsReserveFund() public view {
        assertEq(
            pulleyToken.totalSupply(),
            pulleyToken.reserveFund(),
            "PulleyToken total supply should equal reserve fund"
        );
    }
    ...
    function invariant_AssetReservesBackTokens() public view {
        ...
        assertGe(
            totalReserves,
            totalSupply,
            "Asset reserves should back all minted tokens"
        );
    }
    ...
    function invariant_TradingPoolValueConsistency() public view {
        ...
        assertEq(
            reportedTotalValue,
            actualAssetBalance,
            "Trading pool total value should match asset balances"
        );
    }
    ...
    function invariant_ProfitLossConsistency() public view {
        ...
        assertEq(
            totalProfits,
            handler.ghost_totalProfitsRecorded(),
            "Recorded profits should match ghost variable"
        );
        
        assertEq(
            totalLosses,
            handler.ghost_totalLossesRecorded(),
            "Recorded losses should match ghost variable"
        );
    }
    ...
    function invariant_PendingDistributionBounds() public view {
        ...
        assertLe(
            pendingDistribution,
            totalProfits,
            "Pending distribution should not exceed total profits"
        );
    }
    ...
    function invariant_InsuranceFundsBounds() public view {
        ...
        assertLe(
            insuranceFunds,
            totalSupply,
            "Insurance funds should not exceed total supply"
        );
    }
    ...
    function invariant_TotalBackingConsistency() public view {
        ...
        assertEq(
            reportedTotalBacking,
            totalReserves,
            "Total backing should equal asset reserves"
        );
    }
    ...
    function invariant_NoUnbackedTokens() public view {
        ...
        assertGe(
            totalBacking,
            totalSupply,
            "All tokens should be backed by assets"
        );
    }
    ...
    function invariant_SystemSolvency() public view {
        ...
        uint256 totalAssets = mockUSDC.balanceOf(address(pulleyTokenEngine)) + 
                             mockUSDC.balanceOf(address(tradingPool));
        
        uint256 totalLiabilities = pulleyToken.totalSupply() + 
                                  tradingPool.getTotalPoolValue();
        
        // We allow some flexibility due to profits/losses
        // The key is that we should never have negative equity
        assertGe(
            totalAssets + handler.ghost_totalProfitsRecorded(),
            totalLiabilities,
            "System should maintain solvency"
        );
    }
}
