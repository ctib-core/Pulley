...
contract GatewayTest is Test {
    ...
    function test_Constructor_ZeroAddress() public {
        ...
        new Gateway(
            address(0), address(pulleyTokenEngine), address(tradingPool), crossChain, address(permissionManager)
        );
        ...
        vm.expectRevert(Gateway.Gateway__ZeroAddress.selector);
        new Gateway(
            address(pulleyToken), address(pulleyTokenEngine), address(0), crossChain, address(permissionManager)
        );
